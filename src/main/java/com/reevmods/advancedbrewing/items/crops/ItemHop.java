...
public class ItemHop extends ItemBase implements IHasModel, IPlantable{
	
	// Creates the Item
	public ItemHop(String name) {
		super(name);
	...
	public EnumActionResult onItemUse(EntityPlayer player, World worldIn, BlockPos pos, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
		...
		if(facing == EnumFacing.UP && player.canPlayerEdit(pos.offset(facing),facing, stack) && state.getBlock().canSustainPlant(state, worldIn, pos, EnumFacing.UP, this) && worldIn.isAirBlock(pos.up())){
			worldIn.setBlockState(pos.up(), ModBlocks.HOP_PLANT.getDefaultState());
			stack.shrink(1);
	...
	public IBlockState getPlant(IBlockAccess world, BlockPos pos) {
		return ModBlocks.HOP_PLANT.getDefaultState();
	}
